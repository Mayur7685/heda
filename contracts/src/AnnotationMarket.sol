...
contract AnnotationMarket {
    ...
    function _submitWork(uint256 jobId, uint256 taskId, bytes32 annotationRootHash) internal {
        ...
        _approveWork(job, jobId, taskId);
    }

    function approveBatch(uint256 jobId, uint256[] calldata taskIds) external {
        Job storage job = jobs[jobId];
        require(msg.sender == job.creator, "not creator");
        for (uint256 i = 0; i < taskIds.length; i++) {
            _approveWork(job, jobId, taskIds[i]);
        }
    }

    function _approveWork(Job storage job, uint256 jobId, uint256 taskId) internal {
        Submission storage sub = submissions[jobId][taskId];
        if (sub.exists && !sub.approved) {
            sub.approved = true;
            job.approvedCount++;
    ...
    function rejectWork(uint256 jobId, uint256 taskId) external {
        Job storage job = jobs[jobId];
        require(msg.sender == job.creator, "not creator");
        _rejectWork(jobId, taskId);
    }

    function rejectBatch(uint256 jobId, uint256[] calldata taskIds) external {
        Job storage job = jobs[jobId];
        require(msg.sender == job.creator, "not creator");
        for (uint256 i = 0; i < taskIds.length; i++) {
            _rejectWork(jobId, taskIds[i]);
        }
    }

    function _rejectWork(uint256 jobId, uint256 taskId) internal {
        Submission storage sub = submissions[jobId][taskId];
        if (sub.exists && !sub.approved) {
