...
export function useAnnotationMarket(signer: ethers.Signer | null) {
  return useMemo(() => {
    ...
    return {
      ...
      async approveBatch(jobId: number, taskIds: number[]) {
        const tx = await contract.approveBatch(jobId, taskIds);
        return tx.wait();
      },
      ...
      async rejectBatch(jobId: number, taskIds: number[]) {
        const tx = await contract.rejectBatch(jobId, taskIds);
        return tx.wait();
      },
      ...
      // Read all JobCreated events for the marketplace listing (Indexer API with RPC fallback)
      async listJobs() {
        try {
          const res = await fetch("http://localhost:3001/indexer/jobs");
          if (res.ok) {
